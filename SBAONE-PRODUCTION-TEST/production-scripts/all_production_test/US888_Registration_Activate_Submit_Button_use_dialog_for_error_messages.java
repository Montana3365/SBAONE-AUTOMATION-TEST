...
//Verify all required field search box and click on the submit button to very that user can not submit without entering all required field.		
			...
			System.out.println("User is able to continue to the next page without filling all the required filed, Test Failed");
			}
//Attempt clicking on the submit button and verify if all required filed error out.
			mydrivers.findElement(By.xpath(".//*[@id='submit']")).click();
			Thread.sleep(4000);
			if(mydrivers.getPageSource().contains("Passphrase confirmation is required")){
				System.out.println("The submit button is made active to stimulate required filed when left emty by usser, PASS");
				
			}
			else{
				System.out.println("The submit button is not active to stimulate required filed when left emty by usser, Failed");
		         mydrivers.quit();
			}
			...
			if(mydrivers.getPageSource().contains("I am not a robot' check box must be checked")){
				...
				System.out.println("User did not see error message saying '7 errors prohibited user to submit Registration");
			}		
			
			
	//Step 3. Verify that user is on the registration page
		if(mydrivers.getPageSource().contains("New User Registration")){
			//Step 4. Locate the first name search box and enter a valid first name.
			mydrivers.findElement(By.xpath(".//*[@id='user_first_name']")).sendKeys(FN);		
			//Step 5. Locate the last name search box and enter a valid last name.
			mydrivers.findElement(By.xpath(".//*[@id='user_last_name']")).sendKeys(LN);		
			//Step 6. Locate the Phone Number search box and enter a valid Phone Number.
			...
			//Step 8. Locate the Password search box and enter a valid password as instructed and verify the Strength type.
	...
	//Step 9. Verify that user sees the Terms and condition note .
		if(mydrivers.getPageSource().contains("This is a Federal computer system and is the property of the United States Government")){
	...
	//Step 10. Accept the terms and condition by locating and checking the box on the terms before the submission button.
	...
	Thread.sleep(4000);
	//Step 12. Verify that the reCAPTCHA images are being display and visible to user .
	...
	//Step 13. Verify that that user Sees notification Saying 'an email with verification link was send to users email'.
