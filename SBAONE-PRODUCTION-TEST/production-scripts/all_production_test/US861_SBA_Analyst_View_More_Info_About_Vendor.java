package all_production_test;
	...
	public class US861_SBA_Analyst_View_More_Info_About_Vendor {
	...
	public void US861_SBA_Analyst_View_More_Info_About_Vendor_Setup()throws Exception{
		...
		AAEmail="arun@enquizit.com";
		AAPassW="Be at least 8 characters";
		Duns="023027981";		
		B_N="CEDAR POINT CONSULTING LLC";
	...
	//Clear the search duns number in the search box and very that that the duns number is thesame on the search result.
		...
		Thread.sleep(4000);
	    if(mydriver.getPageSource().contains("023027981")){
	...
	//Clear the search duns number in the search box and very that that the duns number is thesame on the search result.
		mydriver.findElement(By.xpath(".//*[@id='query']")).clear();
		Thread.sleep(4000);
		if(mydriver.getPageSource().contains("CEDAR POINT CONSULTING LLC")){
